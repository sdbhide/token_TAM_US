...
Read this as the index; the theory lives in `ai_inference_tam_theory_v4.md`, an
interactive view of the results lives in `assets/` (open `assets/index.html`),
and the derivations and validations live in the code.
...
**Headline result (under Phase 0 priors).** The full-prior Monte Carlo gives
E[TAM] ≈ **$90B/yr**, median ≈ **$73B**, 90% interval ≈ **$37B to $202B**, with
the mean sitting about 22% above the median (heavy right skew). The deterministic
point calibration lands at ≈ **$42B** (about $18B consumer, $24B enterprise), and
the dashboard's default view (central enterprise levers) reads ≈ **$54B median**,
≈ **$59B mean**. The macro sanity window is the 2026/equilibrium ceiling of
**$40B to $60B**. The single most important number for *reducing* uncertainty:
...
**Presentation layer.** The results are packaged for non-technical readers in
`assets/`: a static single-page dashboard (`index.html`) backed by precomputed
scenarios (`dashboard_data.json`). See `assets/DASHBOARD_README.md` for how it
deploys and regenerates.
...
spend (with optional income and penetration correlation ρ and hard spend caps),
compound-sum moments via Wald plus third-cumulant expansion, whole-market
...
distribution has no closed form; this estimates it accurately and about 150 times
faster than brute force.

### `tam_sensitivity.py`, attribution
Vectorized closed-form evaluators (justified because Phase 2 showed population
noise is ~0.04% of variance, so params to E[TAM] *is* the model), a one-at-a-time
...
66 checks total. The important ones are not "does it run" but "does it match an
...
something that cannot be fudged.

---

## 3. The visualizations

### Phase 2, `tam_phase2_diagnostics.png` (2x2 diagnostics)
**What:** four panels, namely running-mean convergence with ±2 SE, the TAM
...
**Why:** the tornado cannot see interactions or tail-specific drivers; global
sensitivity can.
**Means for the project:** about 95% of variance in the *level* comes from the
four enterprise spend parameters. Moving to the *tail* (P(TAM>$100B)), the S1 to
ST gap widens sharply, so tail risk is driven by parameter *interactions*, and
...
the enterprise-dominated regime the theory predicts, where σ_crit does not depend
...
**What:** posterior-predictive TAM on a log axis with mean, median, and P5 to P95
marked.
**Why:** the single headline figure; the mean and median gap is the lognormal-skew
story.
**Means for the project:** the mean (~$90B) sits about 22% above the median
(~$73B). Reporting a single mean would overstate the typical outcome, so the
distribution, and its interval, is the deliverable.
...
**Means for the project:** the mean now tilts enterprise-leaning after the 2026
re-anchor (roughly 45/55 consumer/enterprise on the point calibration). Variance
...
adoption-rate uncertainty more than 5M firms do. This flags a Phase 0 follow-up,
namely pin the consumer adoption-rate prior. *(Concentration params here are
...
1. **A validated, layered model.** Closed forms (exact), then simulator (matches
   them), then sensitivity (free, because population noise is negligible), then
   figures. 66 independent checks.
2. **The number.** Under current priors, E[TAM] ≈ $90B/yr (median ≈ $73B)
   expected equilibrium TAM, heavily right-skewed, 90% interval $37B to $202B;
   the central point calibration is ≈ $42B and the dashboard default view reads
   ≈ $54B median. The macro sanity window is the 2026/equilibrium ceiling of
   $40B to $60B.
3. **Where the uncertainty lives.** Almost entirely in enterprise spend
   ...
   effort should go.
4. **Honesty about tails.** Concentration is extreme (Gini 0.87; top 1% to 44%),
...
| File | Role |
|------|------|
| `ai_inference_tam_theory_v4.md` | Single source of theory (derivations, corrections) |
| `phase_0.py` | Calibration: priors and macro constraint |
...
| File | Role |
|------|------|
| `assets/index.html` | Static single-page dashboard (React via CDN, no build step) |
| `assets/dashboard_data.json` | 27 precomputed scenarios plus convergence frontier (data for the dashboard) |
| `assets/precompute_dashboard.py` | Regenerates `dashboard_data.json` from the calibrated model |
| `assets/DASHBOARD_README.md` | Dashboard usage, deploy, and regeneration notes |
