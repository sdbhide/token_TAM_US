...
$X$ is **annual USD spend per unit**, as Income distributions are typically published annually.
...
Setting $\rho_s = 0$ recovers the previous formulation exactly. One extra parameter per segment, zero loss of tractability. Note $\rho < 0$ *shrinks* $\sigma_X^2$ — correlation structure directly moves the tail mass that drives §5–§7.
...
(The third central moment is needed for §7; previously only implicitly used its heavy-tail limit.)
...
Previous version treated $E[N]$, $\text{Var}(N)$ as free inputs. We now derive them. Let $\text{Pop}_s$ be the eligible population (US adults; US firms) and $p_s$ the equilibrium adoption probability.
